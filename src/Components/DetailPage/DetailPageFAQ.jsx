...
const DetailPageFAQ = () => {
    ...
    const DesktopFAQ = () => (
        <div className="w-full max-w-[896px] flex flex-col gap-5">
            {faqData.map((faq, index) => (
                <div 
                    key={index}
                    className="shadow-[0px_5px_16px_rgba(8,15,52,0.06)] rounded-[24px] bg-[#F9F9F9] w-full overflow-hidden transition-all duration-300"
                >
                    <div 
                        onClick={() => toggleFaq(index)}
                        className="p-[30px_20px] flex justify-between items-center cursor-pointer"
                    >
                        <div className="leading-7 font-medium text-[22px] text-[#162766]">
                            {faq.question}
                        </div>
                        <div className="flex justify-center items-center w-8 h-8 text-[#f9f9f9] transition-transform duration-300 bg-[#162766] rounded-full">
                            {openFaq === index ? 
    ...
    const MobileFAQ = () => (
        <div className="w-full flex flex-col gap-3">
            {faqData.map((faq, index) => (
                <div 
                    key={index}
                    className="shadow-[0px_3px_10px_rgba(8,15,52,0.06)] rounded-[16px] bg-[#F9F9F9] w-full overflow-hidden transition-all duration-300"
                >
                    <div 
                        onClick={() => toggleFaq(index)}
                        className="p-[15px] flex justify-between items-center cursor-pointer"
                    >
                        <div className="leading-6 font-medium text-[16px] text-[#162766] pr-2">
                            {faq.question}
                        </div>
                        <div className="flex-shrink-0 flex justify-center items-center w-6 h-6 text-[#f9f9f9] transition-transform duration-300 bg-[#162766] rounded-full">
                            {openFaq === index ? 
    ...
    return (
        <div className="w-full relative bg-[#162766] overflow-hidden text-left text-[22px] text-[#F9F9F9] font-['Poppins'] pt-0 pb-5">
            <div className="relative max-w-[1442px] mx-auto px-[15px]">
                ...
                <div className="relative flex flex-col items-center p-2 md:p-5 mt-5 md:mt-10">
                    <div className="flex items-center mb-3 md:mb-5">
                        <h1 className="text-[28px] md:text-[clamp(32px,5vw,45px)] text-[#F9F9F9] text-center font-bold">
